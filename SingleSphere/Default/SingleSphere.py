...
import os
import glob
os.chdir('/storage1/usr/people/cheng/vipr/OomphMercuryCoupling/cmake-build-debug/Drivers/OomphCouplingMultiSolid/SingleSphere/Default')

#Load data in any order
Data = glob.glob('./SingleSphereParticle_*.vtu')
...
for x in range(0,maxTime+1):
	DataSorted.append('./SingleSphereParticle_' + str(x) + '.vtu')
...
walls = XMLUnstructuredGridReader(FileName=glob.glob('./SingleSphereWall_*.vtu'))
Show(walls)
